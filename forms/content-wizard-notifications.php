...
<div class="d4p-wizard-panel-header">
    <p>
		<?php use function Dev4Press\v51\Functions\panel;

		esc_html_e( 'The plugin has Notifications system for instant notifications with additional daily and weekly digests. For each of these notifications types, you need to enable events you want to include, and you can do that from the Events panel where each event has switches for every notification type. You can use special tool to enable or disable notifications for all events at once.', 'coreactivity' ); ?>
        <br/><a target="_blank" href="<?php echo esc_url( panel()->a()->panel_url( 'events', '', '', true ) ); ?>"><?php esc_html_e( 'Events Panel', 'coreactivity' ); ?></a> &middot;
        <a target="_blank" href="<?php echo esc_url( panel()->a()->panel_url( 'tools', 'notifications', '', true ) ); ?>"><?php esc_html_e( 'Bulk Notifications Control', 'coreactivity' ); ?></a>
    </p>
</div>
